...
// The program will tell us the mean color of the pixels selected
    ...
    for(;;)
    {
        ...
        if( !paused )
        {
            ...
            if( trackObject )
            {
                ...
                // Perform meanShift
                // After computing the histogram, we see which image is
                // the closest to the one we selected
                calcBackProject(&hue,1,0,hist,backproj,&phranges);
                backproj &= mask;

                // int iter = meanShift(backproj,trackWindow,TermCriteria(TermCriteria::EPS||TermCriteria::COUNT,100,1));
                RotatedRect trackBox = CamShift(backproj,trackWindow,TermCriteria(TermCriteria::EPS||TermCriteria::COUNT,100,1));
                // Plotting output of tracking
